...
class Moderation(commands.Cog):
    ...
    @commands.command(name="kick", description="Kicks a user from the current server.")
    @commands.guild_only()
    ...
    @commands.command(aliases=["nick"], name="nickname", description="Change nickname of a user from the current server.")
    @commands.guild_only()
    ...
    @commands.command(name="ban", description="Bans a user from the current server.")
    @commands.guild_only()
    ...
    @commands.command(name="massban", description="Mass bans multiple members from the server.")
    @commands.guild_only()
    ...
    @commands.command(name="unban", description="Unbans a user from the current server.")
    @commands.guild_only()
    @permissions.has_permissions(ban_members=True)
    ...
    @commands.command(name="mute", description="Mute a user from the current server.")
    @commands.guild_only()
    ...
    @commands.command(name="unmute", description="Unmute a user from the current server.")
    @commands.guild_only()
    @permissions.has_permissions(manage_roles=True)
    ...
    @commands.command(aliases=["ar"], name="announcerole", description="Makes a role mentionable and removes it whenever you mention the role.")
    @commands.guild_only()
    @permissions.has_permissions(manage_roles=True)
    ...
    @commands.group(name="find", description="Finds a user within a search criteria.")
    @commands.guild_only()
    @permissions.has_permissions(ban_members=True)
    ...
    @find.command(name="playing", description="Finds a user that playing.")
    ...
    @find.command(aliases=["name"], name="username", description="Finds a user by username.")
    ...
    @find.command(aliases=["nick"], name="nickname", description="Finds a user by nickname.")
    ...
    @find.command(name="id", description="Finds a user by id.")
    async def find_id(self, ctx, *, search: int):
        loop = [f"{i} | {i} ({i.id})" for i in ctx.guild.members if (str(search) in str(i.id)) and not i.bot]
        await default.pretty_results(ctx, "name", f"Found **{len(loop)}** on your search for **{search}**", loop)

    @find.command(aliases=["discrim"], name="discriminator", description="Finds a user by discriminator.")
    ...
    @commands.group(name="prune", description="Removes messages from the current server.")
    @commands.guild_only()
    ...
    async def do_removal(self, ctx, limit: int, predicate, *, before: int = None, after: int = None, message: bool = True) -> discord.Message:
        if limit > 2000:
    ...
    @prune.command(name="embeds", description="Removes messages that have embeds in them.")
    async def embeds(self, ctx, search: int = 100):
        await self.do_removal(ctx, search, lambda e: len(e.embeds))

    @prune.command(name="files", description="Removes messages that have attachments in them.")
    async def files(self, ctx, search: int = 100):
        await self.do_removal(ctx, search, lambda e: len(e.attachments))

    @prune.command(name="mentions", description="Removes messages that have mentions in them.")
    ...
    @prune.command(name="images", description="Removes messages that have embeds or attachments.")
    async def images(self, ctx, search: int = 100):
        await self.do_removal(ctx, search, lambda e: len(e.embeds) or len(e.attachments))

    @prune.command(name="all", description="Removes all messages.")
    async def _remove_all(self, ctx, search: int = 100):
        await self.do_removal(ctx, search, lambda e: True)

    @prune.command(name="user", description="Removes all messages by the member.")
    async def user(self, ctx, member: discord.Member, search: int = 100):
        await self.do_removal(ctx, search, lambda e: e.author == member)

    @prune.command(name="contains", description="Removes all messages containing a substring.")
    ...
    @prune.command(name="bots", description="Removes a bot user's messages and messages with their optional prefix..")
    ...
    @prune.command(name="users", description="Removes only user messages.")
    ...
    @prune.command(name="emojis", description="Removes all messages containing custom emoji.")
    ...
    @prune.command(name="reactions", description="Removes all reactions from messages that have them.")
